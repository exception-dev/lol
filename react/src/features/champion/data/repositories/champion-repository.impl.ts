...
import type { ChampionRemoteDataSource } from '@/features/champion/data/data-sources/champion-remote-data-source';
...
function mapDataError(error: unknown): AppError {
  ...
  if (axios.isAxiosError(error)) return new NetworkError(error);
  if (error instanceof ZodError) return new DataError(error);
